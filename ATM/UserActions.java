package ATM;
// Importing necessary classes for handling lists and user input
...
public class UserActions {
    ...
    public static User userLogin(Scanner scanner) {
        ...
        for (User user : ATM.getUserList()) {
            if (user.getUserId().equals(userId) && user.getUserPass().equals(userPass)) {
                ...
                ATM.setCurrentUser(user);// Set the current logged-in user in the ATM system
    ...
    public static User depositAmount(Scanner scanner, User user) {// Get deposit amount
        ...
        System.out.println("Enter notes to deposit into ATM:");
        ...
        if(total != amount){
            ...
            // Display updated ATM note counts
            for (Notes notes : ATM.getNotesList()) {
...
// Handles withdrawal logic for a specific denomination
    public static long performWithdrawal(Notes notes, long amount, ArrayList<String>duplicate){
        ...
        if(Long.parseLong(notes.getNoteName())<=amount && count<=notes.getCount()){
            ...
            notes.setCount(notes.getCount()-count);// Update note count in ATM
            return amount;
        }
    ...
    public static User withdrawAmount(Scanner scanner, User user) throws CloneNotSupportedException {
        ...
        if (amount<=user.getBalance()) {// Check if user has sufficient balance
            ...
            while (amount != 0) {
                ...
                if (amount == 0) {
                    ATM.setNotesList(duplicate);// Update ATM's notes list
                    ...
                    for (Notes notes : ATM.getNotesList()) {
                        System.out.println("ATM.Notes Count: " + notes.getNoteName() + ": " + notes.getCount() + " ");
                    }
                    ATM.setAtmBalance(ATM.getAtmBalance() - money);// Update ATM balance
                    System.out.println("ATM Balance: " + ATM.getAtmBalance());
                    return user;
                }
            }
    ...
    public static User addTransaction(String action, double amount) {
        Transactions transaction = new Transactions(action, amount, ATM.getCurrentUser().getUserId());
    ...
    public static User changePassword(Scanner scanner, User user) {
        ...
        if (!currentPass.equals(user.getUserPass())) {// Validate current password
        ...
        user.setUserPass(newPass);// Update password
